...
class DrawingIssuancePage(ttk.Frame):
    def __init__(self, parent, username="User"):
        ...
        self.col_widths = [150, 80, 100, 200, 150] # Slightly wider action col for 2 buttons
        self.row_widgets = []
    ...
    def _build_ui(self):
        ...
        # Table Container
        self.table_container = tk.Frame(self, bg="white", bd=1, relief="solid", highlightbackground="#e2e8f0")
        ...
        for i in range(self.page_size):
            if i < len(page_data):
                ...
                if i >= len(self.row_widgets):
                    ...
                    issue_btn.pack(side="left", padx=2)
                ...
                vals = [d["no"], d["rev"], d["status"], d["requested_by"]]
    ...
    def refresh(self):
        # Already static, but can reset or re-load if needed
        self._search_data()

    def _search_data(self, *args):
        q = self.search_var.get().lower()
        if q == "search requests..." or not q:
            self.filtered = list(self.drawings)
        else:
            self.filtered = [
